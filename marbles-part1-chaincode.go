...
func (t *SimpleChaincode) Init(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	fmt.Println("init is called: " + function) //log initr
	if len(args) != 1 {
		return nil, errors.New("Incorrect number of arguments. Expecting 1")
	}
	if function == "init_marble" { //initialize the chaincode state, used as reset
		return t.init_marble(stub, args)
	}
	fmt.Println("init did not find func: " + function) //error
...
func (t *SimpleChaincode) Invoke(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	if function == "delete" { //deletes an entity from its state
